...
export const PillFilters = ({ items, selectedId, onSelect, className }: PillFiltersProps) => {
    return (
        <div className={cn("flex cursor gap-3 overflow-x-auto scrollbar-none pb-2", className)}>
            {items.map((item) => (
                <button
                    key={item.id}
                    onClick={() => onSelect(item.id)}
                    className={cn(
                        "px-5 py-2 rounded-full cursor-pointer text-xs font-medium whitespace-nowrap transition-all duration-200",
                        selectedId === item.id
