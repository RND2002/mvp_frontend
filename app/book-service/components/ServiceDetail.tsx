...
export const ServiceDetail: React.FC<ServiceDetailProps> = ({ service, onBack, onProceed }) => {
    ...
    return (
        <>
            <div className="min-h-screen pb-40 lg:pb-24 animate-slide-up bg-bg-primary overflow-x-hidden">
                ...
                <div className="relative w-full h-[200px] md:h-[300px]">
                    ...
                    <div className="absolute top-0 left-0 right-0 p-6 lg:px-12 lg:py-8 flex items-center justify-between z-20">
                        <button
                            onClick={onBack}
                            className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-bg-secondary backdrop-blur-md border border-border-subtle flex items-center justify-center text-text-primary hover:bg-bg-tertiary transition-all active:scale-95 group"
                        >
                    ...
                    <div className="absolute bottom-12 left-6 right-6 lg:left-12 lg:bottom-16 max-w-7xl">
                        <h1 className="text-2xl md:text-4xl font-black text-text-primary mb-2 md:mb-4 uppercase italic tracking-tighter leading-none">
                            {service.label || service.name} <span className="text-[10px] opacity-20">v3</span>
                        </h1>
                        <div className="flex items-center gap-3">
                            <p className="text-theme-amber font-black text-xs md:text-xl uppercase tracking-[0.3em] flex items-center gap-2">
                ...
                <div className="px-4 lg:px-12 max-w-4xl lg:max-w-7xl mx-auto lg:mx-0 space-y-8 -mt-10 lg:-mt-12 relative z-10 lg:grid lg:grid-cols-12 lg:gap-12 lg:space-y-0">
                    ...
                    <div className="lg:col-span-7 space-y-10">
                        ...
                        <div className="hidden lg:block bg-bg-secondary border border-border-subtle rounded-3xl p-8 shadow-xl">
                            <h2 className="text-2xl font-black text-text-primary uppercase tracking-tight mb-4 italic">Service Overview</h2>
                            <p className="text-text-secondary font-medium leading-relaxed text-lg italic">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <div className="lg:sticky lg:top-12 space-y-6">
                            ...
                            <div className="bg-bg-secondary rounded-3xl p-6 border border-border-subtle flex justify-between items-center shadow-2xl relative overflow-hidden group">
                                {selectedVehicle ? (
                                    <div className="flex items-center gap-5 relative z-10">
                                        <div className="w-14 h-14 bg-theme-amber/12 rounded-2xl flex items-center justify-center border border-theme-amber/25">
                                            {selectedVehicle.vehicle_type === VEHICLE_TYPE.TWO_WHEELER ? (
                                                <Bike className="w-7 h-7 text-theme-amber" />
                                            ) : (
                                                <Car className="w-7 h-7 text-theme-amber" />
                                            )}
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black text-text-secondary uppercase tracking-widest mb-1.5">Selected Vehicle</p>
                                            <div className="flex items-center gap-2 mb-1">
                                                <h3 className="text-base font-black text-text-primary uppercase tracking-tight">
                                                    {selectedVehicle.brand} {selectedVehicle.model}
                                                </h3>
                                                <div className="w-1 h-1 rounded-full bg-theme-amber/50"></div>
                                                <span className="text-[10px] font-bold text-theme-amber uppercase tracking-wider">{selectedVehicle.fuel_type}</span>
                                            </div>
                                            <div className="text-text-secondary text-[10px] font-bold tracking-widest uppercase">{selectedVehicle.registration_number || "NO REG DATA"}</div>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex items-center gap-4 relative z-10">
                                        ...
                                        <div>
                                            <h3 className="text-sm font-black text-text-secondary uppercase tracking-widest">No Vehicle Selected</h3>
                                            <p className="text-[10px] text-text-tertiary font-bold uppercase tracking-widest leading-none mt-1">Select from profile</p>
                                        </div>
                                    </div>
                                )}
                                {/* Corner Glow */}
                                <div className="absolute top-0 right-0 w-24 h-24 bg-theme-amber/5 rounded-full blur-2xl -mr-12 -mt-12 group-hover:bg-theme-amber/10 transition-colors"></div>
                            </div>
                            ...
                            {/* Price Card */}
                            <div className="bg-bg-secondary rounded-3xl p-8 border border-border-subtle shadow-2xl relative overflow-hidden group">
                                <div className="relative z-10">
                                    <div className="flex justify-between items-start mb-4">
                                        <span className="text-text-secondary font-black text-[11px] uppercase tracking-[0.2em] flex items-center gap-2">
                                            Total Estimated Price <Info className="w-4 h-4 text-text-tertiary" />
                                        </span>
                                        {isServicesLoading && <Loader size="sm" />}
                                    </div>
                                    <div className="flex items-baseline gap-2 mb-4">
                                        <span className="text-5xl font-black text-text-primary tracking-tighter italic">₹{estimatedPriceMin}</span>
                                        <span className="text-text-secondary font-bold text-xl tracking-tighter italic">— ₹{estimatedPriceMax}</span>
                                    </div>
                                    <div className="p-4 rounded-2xl bg-bg-tertiary border border-border-subtle">
                                        <p className="text-text-secondary text-[10px] font-medium leading-relaxed uppercase tracking-wider">
                                            Final price confirmed after inspection. Base price for <span className="text-theme-amber">{selectedVehicle?.vehicle_type?.replace("_", " ") || "vehicle"}</span>.
                                        </p>
                                    </div>
                                </div>
                                <div className="absolute bottom-0 right-0 w-32 h-32 bg-theme-amber/5 rounded-full blur-3xl -mr-16 -mb-16 group-hover:bg-theme-amber/10 transition-colors"></div>
                            </div>

                            {/* Service Mode Selection */}
                            <div className="space-y-4">
                                <h3 className="text-text-primary font-black text-lg uppercase italic tracking-tighter flex items-center gap-2 px-1">
                                    <Truck className="w-5 h-5 text-theme-amber" />
                                    Choose Service Mode
                                </h3>
                                <div className="grid grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-4">
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            serviceMode === "location"
                                                ? "bg-theme-amber/12 border-theme-amber text-text-primary scale-[1.02]"
                                                : "bg-bg-secondary border-border-subtle text-text-secondary hover:border-border-default"
                                        )}
                                    >
                                        <div className={cn(
                                            "w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300",
                                            serviceMode === "location" ? "bg-theme-amber/20 scale-110" : "bg-bg-tertiary"
                                        )}>
                                            <MapPin className={cn("w-6 h-6", serviceMode === "location" ? "text-theme-amber" : "text-text-tertiary")} />
                                        </div>
                                    ...
                                    >
                                        <div className={cn(
                                            "w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300",
                                            serviceMode === "pickup" ? "bg-theme-amber/20 scale-110" : "bg-bg-tertiary"
                                        )}>
                                            <Truck className={cn("w-6 h-6", serviceMode === "pickup" ? "text-theme-amber" : "text-text-tertiary")} />
                                        </div>
                            ...
                            <div className="hidden lg:block pt-4">
                                <VroomButton
                                    ...
                                    size="lg"
                                    className="w-full h-16 rounded-[6px] text-xl font-semibold uppercase tracking-[0.2em] bg-theme-amber text-black hover:bg-theme-amber-hover border-none"
            ...
            <div className="fixed bottom-[80px] left-0 right-0 p-4 bg-linear-to-t from-background via-background/95 to-transparent z-50 lg:hidden">
                <div className="max-w-4xl mx-auto">
                    <VroomButton
                        ...
                        disabled={!serviceMode || !selectedVehicle || !selectedLocation}
                        size="lg"
                        className="w-full h-16 rounded-[6px] text-lg font-semibold uppercase tracking-wider bg-theme-amber text-black hover:bg-theme-amber-hover border-none"
                        icon={<ChevronRight className="w-6 h-6" />}
                    >
                        {serviceMode ? (
                            <div className="flex items-center gap-3">
